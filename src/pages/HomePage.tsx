...
export default function HomePage() {
  ...
  if (type) {
    ...
    pageTitle = `Phân loại: ${type}`;
    isFiltered = true;
  } else if (slug) {
    displayMovies = movies.filter(m => m.category === slug);
    pageTitle = `Danh mục: ${slug}`;
    isFiltered = true;
  } else if (genre) {
    displayMovies = movies.filter(m => m.genres.split(',').map(g => g.trim()).includes(genre));
    pageTitle = `Thể loại: ${genre}`;
    isFiltered = true;
  } else if (country) {
    displayMovies = movies.filter(m => m.country === country);
    pageTitle = `Quốc gia: ${country}`;
    isFiltered = true;
  }
