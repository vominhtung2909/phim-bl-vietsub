    ...
    return new Promise((resolve, reject) => {
      Papa.parse(csvContent, {
        ...
        complete: (results) => {
          ...
          // Accurate Date Parser for DD/MM/YYYY HH:mm:ss
          const parseSheetDate = (dateStr: string): number => {
            ...
            if (match) {
              ...
              const formattedDate = `${y}-${m}-${d}${rest}`;
              ts = new Date(formattedDate).getTime();
              return isNaN(ts) ? 0 : ts;
            }
            return 0;
          };

          // HARD RESET: CREATE 2 SEPARATE DATA STREAMS
          ...
          const heroData = [...rawMovies].sort((a, b) => {
            ...
            // Tie-breaker: Latest update then ID
          ...
          // 2. List Data: Sorted by updated_at (Strictly Column J)
          const listData = [...rawMovies].sort((a, b) => {
            const tA = parseSheetDate(a.updated_at);
            const tB = parseSheetDate(b.updated_at);
            if (tB !== tA) return tB - tA;
            // Tie-breaker: ID descending (larger ID first)
