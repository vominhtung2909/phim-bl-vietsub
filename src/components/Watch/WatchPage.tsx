...
import { ChevronDown, ChevronLeft, ChevronRight, Clock, User, Tag, Plus, Check, SkipForward, Send, PlayCircle, HelpCircle } from 'lucide-react';
...
export default function WatchPage() {
  ...
  useEffect(() => {
    ...
    const checkWatchlist = async () => {
      try {
        ...
        // Also check localStorage for notification sync
        if (!docSnap.exists()) {
          // If in localStorage but not firebase (e.g. guest or newly added), check that too
          if (notificationService.isInWatchlist(movie.id)) {
  ...
  const toggleWatchlist = async () => {
    ...
    // Always update localStorage first for instant feedback/notification logic
    notificationService.toggleWatchlist(movie);
    
    // If logged in, also sync with Firebase
    if (user) {
      ...
      try {
        ...
        if (isInWatchlist) {
          ...
          await setDoc(docRef, { movieId: movie.id, addedAt: serverTimestamp() });
        }
      } catch (error) {
  ...
  return (
    <div className="min-h-screen pt-24 pb-12">
      ...
      <div className="px-4 md:px-12 max-w-[1200px] mx-auto space-y-6">
            ...
            <div className="flex items-center gap-3">
              <button 
                ...
                className={`flex items-center gap-2 px-6 py-2.5 rounded-full font-bold text-sm transition-all active:scale-95 ${
                  isInWatchlist ? 'bg-zinc-800 text-white' : 'bg-white text-black hover:bg-zinc-200'
                }`}
              >
                {isInWatchlist ? <Check size={18} /> : <Plus size={18} />}
                {isInWatchlist ? 'Đã lưu' : 'Lưu vào danh sách'}
              </button>
            </div>
          </div>
