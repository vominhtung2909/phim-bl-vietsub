...
export default function Hero() {
  ...
  // Hero Data is already precision-sorted by views (Top 10) in service
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="absolute inset-0 bg-[#050505]">
            {/* Backdrop Image - 2/3 width on the right PC, 100% on Mobile */}
            <div className="absolute right-0 top-0 bottom-0 w-full lg:w-[66.666%] z-0">
              {/* Shimmer for Hero */}
              {!imageLoaded[currentIndex] && (
                 <div className="absolute inset-0 bg-zinc-900/50 animate-pulse flex items-center justify-center">
                   <div className="w-32 h-32 bg-purple-600/5 rounded-full blur-[100px]" />
                 </div>
              )}
              ...
              <img 
                ...
                className={`w-full h-full object-cover brightness-[0.4] lg:brightness-[0.9] lg:object-[center_20%] transition-opacity duration-1000 ${imageLoaded[currentIndex] ? 'opacity-100' : 'opacity-0'}`}
                referrerPolicy="no-referrer"
              />
              
              {/* Blend Overlay 1: Left to Right (Black -> Transparent) */}
              <div className="absolute inset-0 bg-gradient-to-r from-[#050505] via-[#050505]/95 lg:via-[#050505]/40 to-transparent z-10" />
              
              {/* Blend Overlay 2: Bottom Blend */}
              <div className="absolute bottom-0 left-0 right-0 h-1/3 bg-gradient-to-t from-[#050505] to-transparent z-10" />
            </div>

            {isDonate && <div className="absolute inset-0 bg-black/20 backdrop-blur-[1px] z-20" />}
          </div>

          {/* Content Area */}
          <div className="relative h-full flex flex-col justify-end px-4 md:px-12 pb-24 md:pb-32 w-full z-30">
            <motion.div
